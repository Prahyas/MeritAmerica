...
public class Exercise04_HotelReservation {
     ...
     * The problems below ask you to implement the logic for determining a guest's
     * total amount for their stay.
     */
    ...
    /*
     * Using the rates from above, implement the logic to determine the total amount
     * based on
    ...
    /*
     * The owners of Innovator's Inn offer parking at an additional cost of $25.00
     * per night.
     ...
     * whether they require parking (boolean), and whether they require a late
     * checkout (boolean).
     *
     * Examples:
    ...
    public double calculateStayTotal(int numOfTotalNights, boolean includesParking, boolean includesLateCheckout) {
        ...
        if (numOfTotalNights >= 3) {
            nightlyRate = DISCOUNT_RATE;
        } else {
            nightlyRate = DAILY_RATE;
        }

        double totalAmoutPrice = numOfTotalNights * nightlyRate;

        if (includesParking) {
            totalAmoutPrice = totalAmoutPrice + (PARKING_RATE * numOfTotalNights);
        }

        if (includesLateCheckout) {
            totalAmoutPrice = totalAmoutPrice + LATE_CHECKOUT_FEE;
        }

        return totalAmoutPrice;
