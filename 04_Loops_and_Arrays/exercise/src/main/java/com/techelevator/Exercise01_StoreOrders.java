...
public class Exercise01_StoreOrders {
    ...
    /*
     * Each customer order, consisting of one or more items, is represented as an
     * array
    ...
    /*
     * Sally realized that she needed to know how many calzones her shop sells per
     * day.
     * 
     * Implement the logic to count the number of calzones sold per day when given
     * an
     * array representing each item that her customers ordered that day.
     * 
     * Examples:
     * getCalzoneSales([CALZONE, LARGE_CHEESE, LARGE_PEPPERONI, CALZONE,
     * SMALL_CHEESE]) → 2
    ...
    /*
     * Sally also needs to know the total revenue for all cheese pizzas sold on any
     * given day.
     * Each small cheese pizza costs $8.
     * Each medium cheese pizza costs $11.
     * Each large cheese pizza costs $14.
     * 
     * Implement the logic to return the total revenue of all cheese pizzas when
     * given
    ...
    public int getCheesePizzaRevenue(int[] orders) {
        int amountForCheesePizza = 0;
        for (int i = 0; i < orders.length; i++) {
            if (orders[i] == SMALL_CHEESE) {
                amountForCheesePizza = amountForCheesePizza + 8;
            }
            if (orders[i] == MEDIUM_CHEESE) {
                amountForCheesePizza = amountForCheesePizza + 11;
            }
            if (orders[i] == LARGE_CHEESE) {
                amountForCheesePizza = amountForCheesePizza + 14;
            }
        }
        return amountForCheesePizza;
    }
}
