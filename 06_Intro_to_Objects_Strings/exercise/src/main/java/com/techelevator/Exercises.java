...
public class Exercises {
     ...
     * string where the word is in the
     * middle of the out string, e.g. "<<word>>". Note: use str.substring(i, j) to
     * extract the String starting
     ...
     * the original string. The string
     ...
     * "Hello" yields "He". If the
     * string is shorter than length 2, return whatever there is, so "X" yields "X",
     * and the empty string ""
     * yields the empty string "". Note that str.length() returns the length of a
     * string.
     * firstTwo("Hello") → "He"
     * firstTwo("abcdefg") → "ab"
     * firstTwo("ab") → "ab"
     ...
     * the shorter string
     * on the outside and the longer string on the inside. The strings will not be
     * the same length, but
    ...
    /*
     * Given a string and an int n, return a string that starts at n and has a
     * length of 2. Note that n may or may not be a valid
     * location in the string. If n is too low or too high to define a string of
     * length 2, return the string's first 2 characters.
     * The string length will be at least 2.
     * twoChar("java", 0) → "ja"
     * twoChar("java", 2) → "va"
     * twoChar("java", 3) → "ja"
     ...
     * string, such as with
     * "badxxx" or "xbadxx" but not "xxbadxx". The string may be any length,
     * including 0. Note: use .equals()
     ...
     * whatever is there if the string is less than length 3. Return n copies of the
     * front;
    ...
    public String frontTimes(String str, int n) {
        ...
        if (n >= 0) {
            for (int i = 0; i < n; i++) {
                if (str.length() >= 3) {
                    fullString = fullString + str.substring(0, 3);

                } else {
                    fullString = fullString + str.substring(0, str.length());
    ...
    public boolean doubleX(String str) {
        if (str.contains("x")) {
            ...
            if (firstIndex < str.length() - 1 && str.charAt(firstIndex + 1) == 'x') {
                return true;
            }
        }
        return false;
     ...
     *
     * We don't count the end subString, so "hixxxhi" yields 1, but the subString
     * may overlap a prior position by one.
     * For instance, "xxxx" has a count of 2: one pair at position 0, and the second
     * at position 1. The third pair at
    ...
    public int last2(String str) {
        ...
        for (int i = 0; i < str.length() - 2; i++) {
            ...
            String currSub = str.substring(i, i + 2);
    ...
    public String stringX(String str) {
        ...
        String replacedStr = str.replaceAll("x", "");
        String result = "";

        // Check if the first character of the original string was 'x'
        if (str.charAt(0) == 'x') {
            result += 'x';  // Add 'x' to the result
        }

        result += replacedStr; // Add the characters without 'x' to the result

        // Check if the last character of the original string was 'x'
    ...
    public String altPairs(String str) {
        ...
        // Iterate through the characters of the string, stepping by 4
        for (int i = 0; i < str.length(); i += 4) {
            // Append the character at the current index
            result += str.charAt(i);
            // Append the character at the next index, if it exists
            if (i + 1 < str.length()) {
